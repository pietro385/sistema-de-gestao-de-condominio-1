...
app.get('/blocos', (req, res) => {
  db.query('SELECT * FROM blocos', (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ erro: 'Erro ao buscar blocos' });
    }
    const blocosFormatados = results.map(bloco => ({
      id: bloco.id_bloco,
      nome: bloco.nome,
      quantidade_apartamentos: bloco.quantidade_apartamentos
    }));
    res.json(blocosFormatados);
  });
});


// Ou altere a criação da tabela para usar 'id' em vez de 'id_bloco'
